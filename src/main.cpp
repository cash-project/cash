...
uint256 hashGenesisBlock("0xf726c77a6da1eec16d60755a0a263450436fdda938044c73bb54c4da5984d75a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cash: starting difficulty is 1 / 2^12
...
{
    // Cash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Cash: 420k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x714214a0af100ca63da09675ce981203bb189d814910fcaf374b602127bfbde2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "the time 28/06/2018 in honor of the creator of bitcoin Satoshi Nakamoto:";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 80 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0477588638bb063dd70d0ef92008d417e82356c3cfbebd9d56717f5f8074b76d18f040d628515253fcdcc2c7d3711b053498ffcb6af709386489666924b140b87d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1530225197;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085621354;

        if (fTestNet)
        {
            block.nTime    = 1530225142;
            block.nNonce   = 386175100;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xee9134e557cf3e948d41c1329bf0a4d625cf49e159938a1d0cfd528e6ec7bf50"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc5, 0xbc, 0xd5 }; // Cash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CashMiner(CWallet *pwallet)
{
    printf("CashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cash-miner");
    ...
    try { loop {
        ...
        printf("Running CashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CashMiner, pwallet));
}
